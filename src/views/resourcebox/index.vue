//GENERATE_START
<!-- The ResourceBox Business Entity manangement Component -->
<template>
  	<div class="app-container">
		...
		<ResourceBoxEditor ref="editor" @onOK="saveEntity"></ResourceBoxEditor>
  	</div>
...
<script>
	...
	import ResourceBoxEditor from '@/views/resourcebox/editor'
  	export default {
		// 注册依赖组件
		components : {ResourceBoxEditor},
    	data() {
      	  	return {
				// 定义表格字段
        		header: [
    				{title: this.$t('resourcebox.state'), field: 'state', class: ''},   
    				{title: this.$t('resourcebox.boxNo'), field: 'boxNo', class: ''},   
    				{title: this.$t('resourcebox.boxId'), field: 'boxId', class: ''}   
        		],
				// 翻页数据格式
				pagination : {
					pageSize: 10,
    	...
    	methods: {
			doSearch() {// 执行查询
				getJson('api/blockchain/resourcebox/paginate', this.conditions, response => {
			...
			saveEntity(){// 保存和修改功能
				...
				if(mode == 'NEW'){
					postJson('api/blockchain/resourcebox', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}else{
					putJson('api/blockchain/resourcebox', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}
			...
			deleteEntity(item){// 删除功能
	            this.$confirm(this.$t('cfgentity.delete_confirm', [item.name]), this.$t('confirm'), {
	              	...
	              	deleteJson('api/blockchain/resourcebox/' + item.boxId, {}, response => {
