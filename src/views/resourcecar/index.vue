...
<template>
  	<div class="app-container">
		...
		<ResourceCarEditor ref="editor" @onOK="saveEntity"></ResourceCarEditor>
  	</div>
...
<script>
	...
	import ResourceCarEditor from '@/views/resourcecar/editor'
  	export default {
		// 注册依赖组件
		components : {ResourceCarEditor},
    	data() {
      	  	return {
				// 定义表格字段
        		header: [
    				{title: this.$t('resourcecar.state'), field: 'state', class: ''},   
    				{title: this.$t('resourcecar.plateNum'), field: 'plateNum', class: ''},   
    				{title: this.$t('resourcecar.carNo'), field: 'carNo', class: ''},   
    				{title: this.$t('resourcecar.carId'), field: 'carId', class: ''}   
        		],
				// 翻页数据格式
				pagination : {
					pageSize: 10,
    	...
    	methods: {
			doSearch() {// 执行查询
				getJson('api/blockchain/resourcecar/paginate', this.conditions, response => {
			...
			saveEntity(){// 保存和修改功能
				...
				if(mode == 'NEW'){
					postJson('api/blockchain/resourcecar', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}else{
					putJson('api/blockchain/resourcecar', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}
			...
			deleteEntity(item){// 删除功能
	            this.$confirm(this.$t('cfgentity.delete_confirm', [item.name]), this.$t('confirm'), {
	              	...
	              	deleteJson('api/blockchain/resourcecar/' + item.carId, {}, response => {
